...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const locationToUse = credentials.geolocation || address;
    
    if (!locationToUse) {
      alert("Please enter an address or click the location button");
      return;
    }
    ...
    const response = await fetch("http://localhost:5000/api/createuser", {
      ...
      body: JSON.stringify({ name: credentials.name, email: credentials.email, password: credentials.password, location: locationToUse })
